...
> Repository development instructions for **gateway** (Service).
...
src/admin, analytics, config, core, graphql, handlers, health, security, subscriptions, types

## Database, APIs and Messaging

- Database: none (no local persistence; relies on downstream services)
- Migrations: none
...
@omnixys/contracts-ts, context-ts, graphql-ts, grpc-ts, http-ts, kafka-ts, logger-ts, observability-ts, security-ts
...
node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; node --test for e2e

## Repository-Specific Rules

Gateway has no database. It aggregates downstream contracts; contract changes here affect every client.
